...
// ============================================================================
// LEGACY: Keep parseTranscriptToStreamMessages for backward compat during migration
...
/**
 * @deprecated Use parseTranscript instead
 */
export function parseTranscriptToStreamMessages(transcriptContent: string): {
  streamMessages: StreamMessage[]
  userMessages: Array<{ id: string; text: string; timestamp: Date }>
} {
  const streamMessages: StreamMessage[] = []
  const userMessages: Array<{ id: string; text: string; timestamp: Date }> = []
  ...
  return { streamMessages, userMessages }
}

/**
 * @deprecated This was the old render-time parser. Use parseAssistantEvent instead.
 */
export function parseClaudeEvents(
  streamMessages: StreamMessage[],
  userMessages: Array<{ id: string; text: string; timestamp: Date }>,
  isStreaming: boolean
): { messages: ParsedMessage[]; currentTodos: TodoItem[] | null } {
  const messages: ParsedMessage[] = []
  const toolResults: Record<string, string> = {}
  ...
  for (const msg of streamMessages) {
    ...
    if (type === 'assistant') {
      // Add the user message that preceded this assistant response
      if (userMessageIndex < userMessages.length) {
        messages.push({
          id: userMessages[userMessageIndex].id,
          role: 'user',
          text: userMessages[userMessageIndex].text,
          timestamp: userMessages[userMessageIndex].timestamp,
        })
        userMessageIndex++
      }

      const result = parseAssistantEvent(event, msg.id)
      if (result) {
        // Apply tool results
        if (result.message.toolCalls) {
          for (const tc of result.message.toolCalls) {
    ...
    if (type === 'user') {
      const results = parseUserEventForToolResults(event)
      for (const r of results) {
        toolResults[r.toolUseId] = r.output
        // Update previous tool calls with results
