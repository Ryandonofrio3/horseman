...
## 🚨 CRITICAL: Will Break Things If Wrong
...
**Flow:**
1. New session → UI ID only, `claudeSessionId = undefined`
2. First message → spawn Claude (no `--resume`)
3. `system` event → extract Claude's `session_id`, store as `claudeSessionId`
4. Follow-ups → `--resume {claudeSessionId}`
...
```js
// ✅ CORRECT - always sync, even to null
useEffect(() => {
  activeClaudeSessionRef.current = claudeSessionId || null
}, [claudeSessionId])

// ❌ WRONG - leaves stale IDs when switching sessions
useEffect(() => {
...
```js
// ✅ CORRECT - ref updated synchronously each render
...
```ts
// ✅ CORRECT - use selectors from src/store/selectors.ts
...
**Why:** Selectors use stable empty arrays (`EMPTY_MESSAGES`). Inline fallbacks like `?? []` create new references every render → Zustand thinks state changed → infinite re-render.
...
// ❌ WRONG - legacy paths, may be removed
...
- `toolsById[toolId]` → the tool object
- `toolMessageIds[toolId]` → which message contains it
...
```
assistant event (tool A) → message 1
assistant event (tool B) → message 2
assistant event (tool C) → message 3
...
1. Edit → DiffDisplay (return!)
2. Write → CodeDisplay (return!)
...
```
Claude wants tool → --permission-prompt-tool intercepts
→ Claude calls mcp__horseman__request_permission
→ horseman-mcp POSTs to Tauri HTTP endpoint
→ Tauri emits { source: 'hook', event: permission_request }
...
1. assistant event { content: [{ type: "tool_use", id: "toolu_xxx", ... }] }
   → parseAssistantEvent creates ToolCall with status: "running"
   → addMessage updates toolsById index
...
4. user event { content: [{ type: "tool_result", tool_use_id: "toolu_xxx", ... }] }
   → updateToolOutput uses toolsById[toolId] for O(1) lookup
   → updates both index and message array
```
...
```ts
// ✅ CORRECT - includes subagent tools from store
const allTools = useAllTools(sessionId)

// ❌ WRONG - misses subagent tools (they're not in any message)
